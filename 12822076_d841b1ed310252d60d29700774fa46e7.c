...
/**								 **/
/**   <Luiza de Jesus Lopes>		  <12822076>   <Matutino-02> **/
/**								 **/
/*********************************************************************/
...
int buscarPalavra(PONT raiz, char* palavra, int n){
  ...
  /*O funcionamento dessa função está ligada principalmente em percorrer os nós de acordo com os caracteres
  da palavra e retorna o valor do contador da última letra da palavra, que indica quantas vezes a palavra foi inserida*/

	int resposta = 0;

	PONT no_atual = raiz;//inicializando com a raiz da trie

	//laço de repetição para percorrer a palavra
    for (int i = 0; i < n; i++) 
	{
		//pegando o indice do caractere no arranjo de filhos
        int indice = palavra[i] - VALOR_a;
        
		//verificando se o nó atual possui filhos e se o nó do indice existe. Caso não existir, retorna 0
		if (!no_atual->filhos || !no_atual->filhos[indice]) 
		{
            return 0;
        }
		//caso o nó exista, o ponteiro é movido para este nó
        no_atual = no_atual->filhos[indice];
    }
	//retornando o número de copias que está registrado agora em 'contador'
	resposta = no_atual->contador;
...
{	/* Complete o codigo desta funcao */ 
	/*A logica aqui é percorrer todos os caracteres da palavra movendo ou criando novos nos no caminho, onde, se necessário, aloca 
	um array de filhos para aumentar a arvore. E no final incrementa o contador do no indicando que uma palavra termina*/

	PONT atual = raiz;
    
	//laço de repetição para percorrer cada caractere da palavra
	for (int i = 0; i < n; i++)
	{	
		//calculando o indice correspondente no array de filhos
        int indice = palavra[i] - VALOR_a;
        
		//caso o no atual nao tenha um array de filhos, cria-se um pra ele
		if (!atual->filhos)
		{
            atual->filhos = (PONT*)calloc(LETRAS, sizeof(PONT));
        }
		//caso nao exista um no pro caractere, cria-se um no pra ele
        if (!atual->filhos[indice]) {
            atual->filhos[indice] = criarNo();
        }
		//avanaçando para o proximo no
...
//função auxiliar recursiva para excluir todas as cópias de uma palavra na trie
bool excluirNo(PONT* no, char* palavra, int n, int pos) 
{
    
	//caso tenha chegado no final da palavra, verifica se a palavra existe
	if (n == pos) 
	{
        if ((*no)->contador > 0) 
		{
            (*no)->contador = 0;//excluindo a palavra
            return true;
        }
        return false;
    }

	//calculando o indice da letra atual no array de filhos
    int indice = palavra[pos] - VALOR_a;
	
	//verificando se o caminho para a letra atual existe
    if (!(*no)->filhos || !(*no)->filhos[indice])
	{
        return false; 
    }

	//chamada recursiva para o proximo no
    bool excluindo = excluirNo(&((*no)->filhos[indice]), palavra, n, pos + 1);

	//verificando se o no filho deve ser excluido
    if (excluindo && !(*no)->contador && !(*no)->filhos[indice]->filhos)
	{
		//liberando espaco na memoria
        free((*no)->filhos[indice]);
		//definindo ponteiro como null
        (*no)->filhos[indice] = NULL;

        if (!(*no)->filhos)
		{

            free((*no)->filhos);//liberando array de filhos
            (*no)->filhos = NULL;
        }

        return true;//resposta positiva: no sera excluido
    }

    return false;//resposta negativa: no nao deve ser excluido
}
...
{ /* Complete o codigo desta funcao */ 
    ...
    //chamando a função recursiva para excluir a palavra
    bool ExcluirRaiz = excluirNo(&raiz, palavra, n, 0);

    //condição para verificar se a raiz deve ser excluída depois da exclusao da palavara
    if (ExcluirRaiz && !raiz->contador && !raiz->filhos)
	{
        free(raiz);//liberando memoria
        raiz = NULL;//deixando o ponteiro da raiz como null
    }


}

//funcao auxiliar recursiva para excluir copia de palavra na trie
bool excluirNoUnica(PONT* no, char* palavra, int n, int pos) 
{
    //caso esteja no final da palavra
    if (n == pos) 
	{
    ...
    //calculando o índice da letra atual no array de filhos
    int indice = palavra[pos] - VALOR_a;
    ...
    //chamando recursiva para o próximo nó
    bool deveExcluir = excluirNoUnica(&((*no)->filhos[indice]), palavra, n, pos + 1);

    //verificando se o nó filho deve ser excluído
    if (deveExcluir && !(*no)->contador && !(*no)->filhos[indice]->filhos) 
	{
        //liberando a memoria do no filho
		free((*no)->filhos[indice]);
        (*no)->filhos[indice] = NULL; 

        //verificando se o array de filhos está vazio
        if (!(*no)->filhos) 
		{
...
{ /* Complete o codigo desta funcao */ 
	/*Essa função funciona também percorrendo os nós de acordo com os caracteres da palavra. Se a palavra existe
	decrementa o contador do nó. Também remove nós desnecessários liberando memória*/
    ...
    //chamando a função recursiva para excluir uma ocorrência da palavra
    bool excluirRaiz = excluirNoUnica(&raiz, palavra, n, 0);

    //caso a raiz deve ser excluída após a exclusão da palavra
    if (excluirRaiz && !raiz->contador && !raiz->filhos) 
	{
        //liberando a memória da raiz se necessário
        if (raiz->filhos) 
		{
            free(raiz->filhos);
            raiz->filhos = NULL;
        }
        //verificando se a raiz pode ser excluída (não é o caso aqui, pois a raiz nunca deve ser excluída)
        //liberando o arranjo de filhos se não houver mais filhos
        if (!raiz->filhos)
		{
            free(raiz->filhos);
            raiz->filhos = NULL;
